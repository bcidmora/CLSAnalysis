...
def PlotSingleHadronCorrelators(the_single_correlator_data, the_type_rs, the_version, the_location, the_rebin, **kwargs):
    ...
    for irrep in s_irreps:
        ...
        vfp.PLOT_CORRELATORS(the_nt, the_mean_corr, the_sigmas_corr, the_rs_scheme, the_nt_ticks, r'$t\,/\,a$', r'$\mathbb{Re}\;C(t)$', 'o', OperatorNamePlot, yscale='log')
        plt.show()
        ...
        vfp.PLOT_HISTOGRAMS(the_rs, r'$\Delta = %s$'%'{:.10e}'.format(the_means_dif) +'\n'+ r'$\sigma = %s$'%'{:.10e}'.format(the_stat_error), the_mean_rs, r'$ \bar{C}_{%s}(t) = $'%the_type_rs + r' $%s$'%'{:.10e}'.format(the_mean_rs), the_nt_mean, r'$ \bar{C}(t) = $ %s'%'{:.10e}'.format(the_nt_mean), OperatorNamePlot + rf' $\to t = {tt+the_nt[0]} a$', the_nr_bins,  'Correlator')
        plt.show()
